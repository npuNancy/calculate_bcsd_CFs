"""
中国区域逐3小时光伏容量因子计算（CMIP6 BCSD rsds + tas + sfcWind）
================================================================

本脚本用于基于 CMIP6 BCSD 降尺度数据计算中国区域光伏容量因子。

计算逻辑：
  1. 从 BCSD 数据中读取 rsds、tas、sfcWind 三个变量；
  2. rsds 作为 GHI，单位自动转换为 kW m-2；
  3. tas 自动转换为摄氏度；
  4. sfcWind 直接作为近地面风速，用于光伏组件温度修正；
  5. 以 rsds 的半点时间轴为主，将瞬时变量 tas/sfcWind 线性插值到 rsds 时间；
...
输出：
  {output_dir}/{model}/solar_CF_china_{model}_{scenario}_{years}_{months}.nc

输出变量：
  - solar_cf(time, lat, lon)，单位 1，范围 [0, 1]

注意：
  - 中国区域数据没有 uas/vas，使用 sfcWind（10m 合成风速）替代。
  - 时间对齐采用 scheme4：保留 rsds 时间轴，将 tas/sfcWind 插值到 rsds 时间。
  - 各模型 tas/sfcWind 起始时间戳不同（00:00/01:30/03:00），rsds 统一从 01:30 开始。
  - 输出时间分辨率与 rsds 输入一致（3 小时）。
"""
...
def compute_diffuse_fraction_erbs(
    ...
    """Erbs 辐射分解：GHI -> diffuse fraction。返回 diffuse fraction 和 cos(SZA)。"""
...
def paper_temperature_coefficient(
    ...
    """论文温度修正：TEMcoef = 1 - gamma * (Tcell - T_STC)。"""
...
def compute_solar_cf_chunk(
    ...
    wind_speed = sfcWind.astype(np.float32)
...
def _datetime64_to_ns(values: np.ndarray) -> np.ndarray:
    ...
    if arr.dtype == object and arr.size > 0:
        ...
        if hasattr(sample, "year"):
            return np.asarray(
                cftime.date2num(arr, units="days since 1850-01-01", calendar="standard"),
                dtype=np.float64,
            )
...
def create_output_file(
    ...
    cf.long_name = "Solar photovoltaic capacity factor (China)"
    cf.units = "1"
    cf.description = (
        "PV capacity factor for China computed from rsds/tas/sfcWind using Erbs diffuse fraction, "
...
def compute_china_solar_cf(
    ...
    """计算中国区域光伏容量因子。"""
    rsds_file = find_china_bcsd_file(data_dir, model, scenario, "rsds", years)
    tas_file = find_china_bcsd_file(data_dir, model, scenario, "tas", years)
    ...
    out_file = (
        Path(output_dir) / model / f"solar_CF_china_{model}_{scenario}_{years}_{months_tag(months)}.nc"
    )
    ...
    n_time_before = {
        ...
        "sfcWind": ds_wind.sizes[time_name],
    }
    logger.info(f"输入时间步数量：{n_time_before}")
    logger.info("时间对齐策略：保留 rsds.time，将 tas/sfcWind 线性插值到 rsds 时间（边界用最近值）。")
    ...
    attrs = {
        ...
        "source_files": ", ".join(str(p) for p in [rsds_file, tas_file, sfcWind_file]),
        "time_alignment": "scheme4: keep rsds time axis; linearly interpolate instantaneous tas/sfcWind to rsds time; boundary uses nearest value",
        "radiation_decomposition": "Erbs diffuse fraction from rsds/GHI",
        ...
        "wind_speed_source": "sfcWind (10m wind speed, not uas/vas)",
    ...
    try:
        for start in tqdm(range(0, n_time, chunk_time), desc=f"{model}-{scenario}", unit="块"):
            ...
            wind_raw = interp_instantaneous_to_target_times_chunk(
                ds_wind[wind_var], time_name, target_times, fill_boundary="nearest"
            )

            rsds_kw = to_kw_m2(rsds_raw, rsds_units)
            tas_c = tas_to_celsius(tas_raw, tas_units)
            sfcWind = wind_raw.astype(np.float32)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CMIP6 BCSD 中国区域光伏容量因子计算（rsds + tas + sfcWind，Erbs + 双轴跟踪 + 论文温度修正）"
    )
    ...
    parser.add_argument("--output_dir", default="output/CFs_of_solar_china", help="输出目录")
...
python S03E01_Simulate_Solar_CF_china.py \
