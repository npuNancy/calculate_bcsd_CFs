...
本次执行前已确认 CF campaign 作业已停止，因此不存在代码更新改变正在运行任务输入模式的问题。正式切换前仍应保留版本化 pilot 目录和等价性记录，用于审计 `.blocks` schema、空间匹配、MaxRSS 和读写吞吐。
...
## 11. 乌镇199隔离 pilot 实测结果（2026-09-14）

测试固定使用 `BCC-CSM2-MR / ssp126 / solar / R03C09`，输入为共享 `production_v1` 的 8 个 BCSD point block，输出写入 `/work/home/aclym5felp/cf_block_pilot_20260914/`，未写入正式 CF 输出目录。四组作业均以 `input_mode=blocks` 成功完成，最终文件均包含 134320 个时间点、8 个测试场站；`n=2、4、8` 相对 `n=1` 的最终 CF 数组逐点完全一致（最大绝对差 0）。

| 进程数 | Slurm 作业 | walltime | MaxRSS | 结果 |
|---:|---:|---:|---:|---|
| 1 | 44297938 | 3:52 | 630 MiB | 成功 |
| 2 | 44298722 | 2:18 | 1.26 GiB | 成功 |
| 4 | 44299521 | 0:37 | 2.22 GiB | 成功 |
| 8 | 44299861 | 1:14 | 4.14 GiB | 成功 |

本次样本显示 `n=4` 的 walltime 最优；`n=8` 虽然仍比 `n=1` 快，但受共享存储 I/O 和内存增长影响没有继续加速。因此正式 campaign 不应直接固定 8 进程，建议先以 `n=4` 作为候选默认值，并在更大场站数、重复运行和节点级监控下复测。
